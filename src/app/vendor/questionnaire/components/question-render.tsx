...
// Define the UploadedFile interface to match FileUpload expectations
...
export const QuestionRenderer: React.FC<QuestionRendererProps> = ({
  ...
  // Helper function to convert legacy string[] evidenceFiles to UploadedFile[]
  const convertEvidenceFiles = (evidenceFiles: any): UploadedFile[] => {
    if (!evidenceFiles) return []
    
    // If it's already an array of UploadedFile objects, return as is
    if (Array.isArray(evidenceFiles) && evidenceFiles.length > 0 && typeof evidenceFiles[0] === 'object' && evidenceFiles[0].fileName) {
      return evidenceFiles as UploadedFile[]
    }
    
    // If it's an array of strings (legacy format), convert to UploadedFile format
    if (Array.isArray(evidenceFiles) && evidenceFiles.length > 0 && typeof evidenceFiles[0] === 'string') {
      return evidenceFiles.map((url: string, index: number) => ({
  ...
  // Handle evidence files change from FileUpload component
  const handleEvidenceFilesChange = (files: UploadedFile[]) => {
    onResponseChange(question.id, { evidenceFiles: files })
  }

  // Legacy file upload handler for FILE_UPLOAD question type
  const handleFileUpload = async (files: FileList | null) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Convert existing files if they're in legacy format
        const existingFiles = convertEvidenceFiles(currentFiles)
        const newFiles = [...existingFiles, newFile]
  ...
  const removeFile = (fileIndex: number) => {
    const currentFiles = convertEvidenceFiles(response?.evidenceFiles || [])
  ...
  const renderInput = () => {
    ...
    switch (question.questionType) {
                ...
                >
                  ...
                  <Label htmlFor={`${question.id}-${index}`} className="flex-1 cursor-pointer">
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-gray-800">{choice.label || choice.value}</span>
                      {choice.points && (
      ...
      case 'FILE_UPLOAD':
        const evidenceFiles = convertEvidenceFiles(response?.evidenceFiles || [])
        return (
          <div className="space-y-4">
            <div
                ...
                >
                  <div className="space-y-4">
                    ...
                    <div className="p-4 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-lg">
                      <Label className="text-sm font-semibold text-amber-900 mb-4 flex items-center gap-2">
                      ...
                      <FileUpload
                        ...
                        files={convertEvidenceFiles(response?.evidenceFiles || [])}
